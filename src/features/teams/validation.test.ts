...
const valid: TeamFormValues = { name: "Dragons", playerCount: "5", region: "Asia", country: "Myanmar" };
...
describe("validateTeamForm", () => {
  ...
  it("rejects names already taken, ignoring case and surrounding whitespace", () => {
    expect(validateTeamForm({ ...valid, name: "  dRAGONS " }, [existing]).name).toMatch(/already exists/);
  });
  ...
  it("bounds the player count to whole numbers within range", () => {
    ...
    expect(validateTeamForm({ ...valid, playerCount: String(PLAYER_COUNT_MAX + 1) }, []).playerCount).toBeDefined();
  ...
  it("does not allow shrinking a team below its current roster", () => {
    const roster = [1, 2, 3].map((id) => ({ id, firstName: "P", lastName: "T", position: "G", nbaTeam: "" }));
    const team = { ...existing, players: roster };
    expect(validateTeamForm({ ...valid, playerCount: "2" }, [team], team).playerCount).toMatch(/already has 3/);
